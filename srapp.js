...
const { bot_Devs, client_token, guıld_VoiceChannelID, guıld_logChannelID, emoji_tik, emoji_red, bot_prefix, emoji_star, bot_status } = require('./sraconfig.json');
...
if (msg.content === (`${bot_prefix}ping`) && msg.author.id === bot_Devs){
msg.channel.send(new Discord.MessageEmbed()
.setColor("RANDOM")
.addField(`${emoji_star} Pingim` , `**${client.ws.ping}** MS`)
  )}})
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(`${bot_prefix}eval`) && message.author.id === bot_Devs) {
    let args = message.content.split(' ').slice(1);
    if (!args[0]) return message.channel.send(`${emoji_red} Kod belirtilmedi`);
      let code = args.join(' ');
      function clean(text) {
...
client.login(client_token).then(c => console.log(`${client.user.tag} [BOT] Bot connected!`)).catch(err => console.error("[BOT] Bot can't connected!"));
